defmodule Bubblit.BubbleRoomsTest do
  ...
  describe "room_actions" do
    ...
    @valid_attrs %{param: "some param", sub_type: 42, type: 42}
    @update_attrs %{param: "some updated param", sub_type: 43, type: 43}
    @invalid_attrs %{param: nil, sub_type: nil, type: nil}
    ...
    test "create_room_action/1 with valid data creates a room_action" do
      ...
      assert room_action.sub_type == 42
    ...
    test "update_room_action/2 with valid data updates the room_action" do
      room_action = room_action_fixture()
      assert {:ok, %RoomAction{} = room_action} = BubbleRooms.update_room_action(room_action, @update_attrs)
      assert room_action.param == "some updated param"
      assert room_action.sub_type == 43
      assert room_action.type == 43
    end

    test "update_room_action/2 with invalid data returns error changeset" do
      room_action = room_action_fixture()
      assert {:error, %Ecto.Changeset{}} = BubbleRooms.update_room_action(room_action, @invalid_attrs)
